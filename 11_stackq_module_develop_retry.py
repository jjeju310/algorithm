def solution(progresses, speeds):
    ...
    left_progresses = [100-p for p in progresses]
    left_day = [int(p/speeds[idx]) if (p%speeds[idx]) == 0 else int(p/speeds[idx])+1 for idx, p in enumerate(left_progresses) ]
    
    max_num = left_day[0]
    answer = [1]
    answer_idx = 0
    
    for n in range(1, len(left_day)):
        if left_day[n] >max_num:
            answer_idx += 1
            answer.append(1)
            max_num = left_day[n]
        else:
